...
- 将本地数据迁移到服务器 首先在你的mongodb的bin目录下查看是否有mongodump程序，如果没有去官网下载工具集。使用mongodump -d 数据库名 备份本地数据，他会生成一个文件夹dump，将这个文件夹上传到服务器的/root文件夹下，在服务器调用mongorestore命令，恢复数据到服务器.
- bcrypt在linux下安装可能会报错。但是因为你可能是npm i 发现不了，这样在你登录验证的时候就会出现问题，因为你使用的，pm2挂起./bin/www文件，你看不到服务器的错误从而导致服务器长时间没有信息 在超过请求最大时间返回给你504的错误。但是也有可能是请求时间太短，服务器还没有处理完你请求的数据，没来得及给你返回数据。
如果你是因为bcrypt没有成功安装引起的错误，那么这样解决:
```js
1.检查你的node版本 bcrypt可能因为node版本太高不兼容
2.检查bcrypt版本 高版本的bcrypt也可能不兼容低/高版本的nodejs。我的node版本是 8.11.3 bcrypt版本是 3.0.0
```