<template>
  <div class="about">
    ...
    <el-table :data="items" border>
      ...
      <el-table-column prop="categories[0].name" label="文章分类" width="120">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    remove(row){
      this.$confirm(`此操作将删除文章${row.title}, 是否继续?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(async () => {
          let {data} = await this.$http.delete(`/rest/articles/${row._id}`)
          console.log(data);
          if(data.state === 'ok'){
            this.$message({
              type: 'success',
